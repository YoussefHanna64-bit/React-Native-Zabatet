...
export function computeSprintStatus(sprint: {
  startDate: string;
  endDate: string;
...
export function useDashboardData(projectId?: string) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          memData.owner &&
          !membersList.find((m: MemberDoc) => m._id === oId)
        ) {
          membersList.unshift({
            ...memData.owner,
            role: "Owner",
  ...
  const kpis = useMemo(() => {
    ...
    const activeSprints = allSprints.filter(
      (s) => computeSprintStatus(s) === "Active",
    ).length;
  ...
  const activeSprintsList = useMemo(() => {
    return allSprints
      .filter((s) => computeSprintStatus(s) === "Active")
      .map((sprint) => {
        const sprintTasks = allTasks.filter((t) => {
          const sid =
            typeof t.sprintId === "object" && t.sprintId
              ? t.sprintId._id
              : t.sprintId;
        ...
        const totalPts = sprintTasks.reduce(
          (s, t) => s + (t.storyPoints ?? 0),
          0,
        );
